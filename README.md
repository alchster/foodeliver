...
Предполагается, что компилятор Go установлен, а так же путь $GOPATH/bin прописан в системных путях поиска бинарника. Для posix систем и Windows это переменная PATH. Значение GOPATH можно посмотреть командой `go env`.
